...
$(document).ready(function () {
    ...
    // Submit Single Question button on Click
    $("#submitAnswer").click(function () {
        ...
        if (isMarked) {
            ...
            alert("You Must answer first!");
        }

    });

    var submitQuestion = function (qNumber) {

        var rightAnswers = new Array();
        rightAnswers[0] = "true";
        rightAnswers[1] = "false";
        rightAnswers[2] = "option2";
        rightAnswers[3] = "option3";
        rightAnswers[4] = ["option1"];
        rightAnswers[5] = ["option1", "option2", "option3"];
        rightAnswers[6] = "val";
        rightAnswers[7] = "html";
        ...
        if (qNumber <= 4) {
            ...
            if ($("input[name='answer" + qNumber + "']:checked").val() === rightAnswers[qNumber - 1]) {
                //score++;
                $("#answer" + qNumber).text("Right answer!");

            } else {
                $("#answer" + qNumber).css("background-color", "#F66");
                $("#answer" + qNumber).text("Right answer is " + rightAnswers[qNumber - 1]);
            }

            $("input[name='answer" + qNumber + "']").prop('disabled', true);
            $("#answer" + qNumber).show();
        } else if (qNumber <= 6) {

            $("#answer" + qNumber).css("background-color", "#FC9");
            $("#answer" + qNumber).text("Right answers are " + rightAnswers[qNumber - 1].join(", "));
            $("input[name='answer" + qNumber + "']").prop('disabled', true);
            $("#answer" + qNumber).show();

        } else {

            if ($("input[name='answer" + qNumber + "']").val().toLowerCase() === rightAnswers[qNumber - 1].toLowerCase()) {
                $("#answer" + qNumber).text("Right answer!");
    ...
    $("#submit").click(function () {
        ...
        if (confirm("Are you sure? you want to submit and end quiz.")) {
            ...
            if (multichoice.length <= rightAnswer5.length) {
                ...
                $("#answer5").css("background-color", "#F66");
            }
            ...
            if (multichoice.length <= rightAnswer6.length) {
                ...
                $("#answer6").css("background-color", "#F66");
            }
